...
impl<B> MakeSpan<B> for SpanCreator {
    fn make_span(&mut self, req: &axum::http::Request<B>) -> tracing::Span {
        ...
        let span = tracing::info_span!(
            ...
            "http.request.header.content-length" = ?headers.get("Content-Length"),
            "http.request.header.content-type" = ?headers.get("Content-Type"),
            "http.request.header.user-agent" = ?headers.get("User-Agent"),
